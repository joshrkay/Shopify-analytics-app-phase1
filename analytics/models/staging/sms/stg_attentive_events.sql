{{
    config(
        ...
        enabled=var('enable_attentive', true)
    )
}}

{#
    Staging model for Attentive SMS events with normalized fields and tenant isolation.

    This model:
    - Extracts and normalizes raw Attentive SMS event data from Airbyte
    - Tracks message-level SMS events (sent, delivered, clicked, etc.)
    - Supports campaign and journey attribution
    ...
    - campaign_id, campaign_name, journey_id, journey_name
...
{% if not source_exists('airbyte_raw', '_airbyte_raw_attentive_events') %}
...
select
    ...
    cast(null as text) as journey_id,
    cast(null as text) as journey_name,
    cast(null as text) as journey_step_id,
    ...
    cast(null as text) as segment_id,
...
with raw_attentive_events as (
    select
        _airbyte_ab_id as airbyte_record_id,
        _airbyte_emitted_at as airbyte_emitted_at,
        _airbyte_data as event_data
    from {{ source('airbyte_raw', '_airbyte_raw_attentive_events') }}
    {% if is_incremental() %}
    where _airbyte_emitted_at >= current_timestamp - interval '{{ var("attentive_lookback_days", 3) }} days'
    {% endif %}
),

attentive_events_extracted as (
    select
        raw.airbyte_record_id,
        raw.airbyte_emitted_at,
        raw.event_data->>'id' as event_id_raw,
        raw.event_data->>'type' as event_type_raw,
        raw.event_data->>'timestamp' as event_timestamp_raw,
        -- Subscriber info
        ...
        -- Journey attribution
        raw.event_data->'message'->>'journey_id' as journey_id_raw,
        raw.event_data->'message'->>'journey_name' as journey_name,
        raw.event_data->'message'->>'journey_step_id' as journey_step_id,
        -- Message details
        raw.event_data->'message'->>'text' as message_text,
        raw.event_data->'message'->>'link_url' as clicked_url,
        raw.event_data->'message'->>'segment_id' as segment_id,
        raw.event_data->'message'->>'segment_name' as segment_name,
        -- Delivery info
        raw.event_data->'delivery'->>'status' as delivery_status,
        raw.event_data->'delivery'->>'error_code' as error_code,
        raw.event_data->'delivery'->>'carrier' as carrier,
        -- Revenue attribution
        raw.event_data->'conversion'->>'revenue' as revenue_raw,
        raw.event_data->'conversion'->>'currency' as currency_code,
        raw.event_data->'conversion'->>'order_id' as order_id
    from raw_attentive_events raw
),

attentive_events_normalized as (
    select
        -- Event identifiers
        case
        ...
        case
            ...
            when lower(event_type_raw) like '%opt%out%' or lower(event_type_raw) like '%unsubscribe%' then 'opted_out'
            when lower(event_type_raw) like '%conversion%' or lower(event_type_raw) like '%purchase%' then 'converted'
        ...
        case
            ...
            when event_timestamp_raw ~ '^\d+$'
                then to_timestamp(event_timestamp_raw::bigint / 1000)
            else null
        ...
        case
            ...
            when event_timestamp_raw ~ '^\d+$'
                then to_timestamp(event_timestamp_raw::bigint / 1000)::date
            else null
        end as report_date,
        ...
        -- Journey attribution
        case
            when journey_id_raw is null or trim(journey_id_raw) = '' then null
            else trim(journey_id_raw)
        end as journey_id,
        journey_name,
        journey_step_id,
        ...
        case
            ...
            when journey_id_raw is not null and trim(journey_id_raw) != '' then 'journey'
            else 'other'
    ...
    from attentive_events_extracted
),

-- Join to tenant mapping to get tenant_id
attentive_events_with_tenant as (
    select
        events.*,
        coalesce(
            (select tenant_id
             from {{ ref('_tenant_airbyte_connections') }}
             where source_type = 'source-attentive'
               and status = 'active'
               and is_enabled = true
             limit 1),
            null
        ) as tenant_id
    from attentive_events_normalized events
),

-- Add internal IDs and canonical channel
attentive_events_final as (
    select
        ...
        case
            ...
            when journey_id is not null then
                {{ generate_internal_id('tenant_id', 'source', 'journey_id') }}
            else null
    ...
    from attentive_events_with_tenant
)
