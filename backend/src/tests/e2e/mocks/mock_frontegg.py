"""
Mock Frontegg authentication server for E2E testing.
...
class MockFronteggServer:
    """
    Mock Frontegg authentication server.
    ...
    def __init__(self, key_id: str = "test-key-1"):
        """
        Initialize mock Frontegg server with RSA key pair.
    ...
    def create_test_token(
        ...
        # Build claims
        claims = {
            "sub": user_id or f"user-{uuid.uuid4().hex[:8]}",
            "email": email or f"test-{uuid.uuid4().hex[:8]}@example.com",
            "org_id": tenant_id,  # This is how Frontegg provides tenant context
            "roles": roles or ["user"],
    ...
    def get_jwks(self) -> Dict:
        ...
        This is what the application fetches from Frontegg to verify tokens.
        """
...
def decode_test_token(token: str, verify: bool = False) -> Dict:
    ...
    """
    if verify:
        raise NotImplementedError("Use MockFronteggServer for verification")
